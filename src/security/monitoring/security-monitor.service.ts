...
export class SecurityMonitorService {
  ...
  async monitorTransaction(transactionData: any): Promise<void> {
    try {
      // Check for anomalies
      const anomalies = await this.anomalyDetector.detectAnomalies(transactionData);
      ...
      const washTradingResult = await this.fraudDetector.detectWashTrading(transactionData);
  ...
  async createSecurityEvent(eventData: Partial<SecurityEvent>): Promise<SecurityEvent> {
    ...
    this.logger.log(`Security event created: ${eventData.eventType} - ${eventData.description}`);
    
    // Trigger alerts for high/critical severity
    if ([SeverityLevel.HIGH, SeverityLevel.CRITICAL].includes(eventData.severity)) {
  ...
  async triggerAlert(event: SecurityEvent): Promise<void> {
    ...
    this.logger.warn(`ALERT TRIGGERED: ${event.eventType} - ${event.description}`);
  ...
  async resolveEvent(eventId: string, resolvedBy: string): Promise<SecurityEvent> {
    const event = await this.securityEventRepo.findOne({ where: { id: eventId } });
    if (!event) {
  ...
  async getSecurityMetrics(timeRange: { start: Date; end: Date }): Promise<any> {
    ...
    return {
      ...
      averageConfidenceScore: events.reduce((acc, e) => acc + e.getConfidenceScore(), 0) / events.length || 0,
    };
  }
