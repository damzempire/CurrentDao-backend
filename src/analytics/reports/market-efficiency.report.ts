...
export class MarketEfficiencyReport {
  ...
  async generateReport(params: ReportParamsDto): Promise<MarketEfficiencyReport> {
  ...
  private calculatePriceDiscoveryEfficiency(metrics: MarketEfficiencyMetrics[]): number {
  ...
  async generateRealTimeMetrics(gridZoneId?: string, country?: string): Promise<MarketEfficiencyMetrics> {
    const queryBuilder = this.analyticsRepository
      .createQueryBuilder('analytics')
      .where('analytics.type = :type', { type: AnalyticsType.MARKET_EFFICIENCY })
