...
export class StreamProcessorService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initializeKafka() {
    this.kafka = new Kafka({
      clientId: 'real-time-analytics-stream-processor',
      brokers: this.configService.get<string[]>('KAFKA_BROKERS', ['localhost:9092']),
    });

    this.producer = this.kafka.producer();
    this.consumer = this.kafka.consumer({
      groupId: 'real-time-analytics-group',
    });
  ...
  private startStreamProcessing() {
    ...
    }, 50); // Process every 50ms for <100ms latency requirement
  }
  ...
  private async processEventBuffer() {
    ...
    const batchSize = Math.min(this.eventBuffer.length, 200); // Process in batches
  ...
  private async processEnergyTradingEvent(event: StreamEvent) {
    ...
    await this.redis.zadd(
      ...
      JSON.stringify({
        eventId: event.id,
        volume: (event.data.volume || 0).toString(),
  ...
  private async processUserActivityEvent(event: StreamEvent) {
    ...
    const processingTime = Math.random() * 15 + 5; // 5-20ms
    await this.delay(processingTime);
  ...
  async getHealthCheck() {
    return {
      ...
      services: {
        kafka: this.producer && this.consumer ? 'connected' : 'disconnected',
        redis: this.redis.status === 'ready' ? 'connected' : 'disconnected',
        eventBuffer: this.eventBuffer.length < 1000 ? 'healthy' : 'overloaded',
      },
      metrics: {
  ...
  private async getTotalUserActions(userKeys: string[]): Promise<number> {
    ...
    return results.reduce((sum, [err, count]) => sum + parseInt((count as string) || '0'), 0);
  }
}
